...
fn eval(s: Result<MalType, String>, env: MalEnv) -> Result<MalType, String> {
    match s? {
        MalType::List(ref l) if l.is_empty() => Ok(MalType::List(l.to_vec())),
...
fn eval_ast(ast: &MalType, env: MalEnv) -> Result<MalType, String> {
    match ast {
        ...
        MalType::List(l) => Ok(MalType::List(eval_vec_elemwise(l, env)?)),
        MalType::Vector(v) => Ok(MalType::Vector(eval_vec_elemwise(v, env)?)),
        _ => Ok(ast.clone()),
    }
}
...
fn eval_vec_elemwise(vec: &[MalType], env: MalEnv) -> Result<Vec<MalType>, String> {
    vec.iter()
        .map(|elem| eval(Ok(elem.clone()), env.clone()))
...
fn rep(input: &str, env: MalEnv) -> String {
